...
class CompetitorsMath extends Component {
    ...
    get requiredParams() {
        return assign({}, super.requiredParams, {
            domain: "domain.competitors.domains." + this.state.domainIndex,
        });
	}
	...
	changeTab = (index) => {
		if (index !== this.state.domainIndex ) {
			this.setState({
				domainIndex: index,
				filterRange: []
			}, this.refresh);
	...
	renderTabs() {
		return(
			<div className="competitors-tabs">
				{this.state.domainsList.map((competitor, key) => {
					return (
						<div
							key={key} 
							onClick={() => this.changeTab(key)} 
							className={ `competitors-tabs__tab  ${this.state.domainIndex == key ? "competitors-tabs__tab--active" : ""}`}
						>
							{competitor}
						</div>
					);
                ...
                <div className="modal-title-group">
                    ...
                    <div className="modal-title-group__description">
                        <div className="modal-title">
                            <h2>
                                <span>{this.trans.title_2} {this.state.domainsList[this.state.domainIndex]}</span>
                                {
                            ...
                            <p className="tagline">{this.trans.we_found_part_1} {this.state.entries.length} {this.trans.we_found_part_2_alt} {this.state.domainsList[this.state.domainIndex]}</p>
...
module.exports = CompetitorsMath;

...
class NewLinks extends Component {
	...
	get name() {
		return 'refdomains_top_new';
	}

	get sort_collection(){
        return this.state.refdomains_top_new;
    }
				...
				<div className="links-activity__wrapper">
                    ...
                    <div className="links-activity__table">
    					<table>
							...
							/>
							<tbody>
								{links && links.length > 0 ? pages[this.state.page - 1].map(this.renderRows) : null}
							</tbody>
    					</table>
						{
							pages.length > 1
